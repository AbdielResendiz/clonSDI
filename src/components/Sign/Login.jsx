  ...
  return (
    <View flex={3} bg='white'>
      <Center flex={1}>
        <Box mt={4} bg='white'>
          <Container>
            <Image
              source={{
                uri: `https://sdiqro.com/wp-content/uploads/2022/05/sdi-logo.png`,
              }}
              alt=''
              w={300}
              h={200}
      ...
      <View flex={2} bg='white' pb={20}>
        ...
        {JSON.stringify(isShow) === JSON.stringify(login) ? (
          <FormularioLogin />
        ) : null}
      </View>
    </View>
        ...
        {JSON.stringify(isShow) === JSON.stringify(start) ? (
          <View flexDirection={'row'} justifyContent={'space-evenly'}>
            <Box>
              <TouchableOpacity
                Opacity
                onPress={() => setShow({ vista: 0, status: true })}
              >
                <Center
                  bg='#white'
                  h={'41px'}
                  w={'80px'}
                  borderBottomColor={'#FE308E'}
                  borderBottomWidth={'3px'}
                >
                  <Text color={'#FE308E'}>Registro</Text>
                </Center>
              </TouchableOpacity>
            </Box>
            <Box>
              <TouchableOpacity
                Opacity
                onPress={() => setShow({ vista: 2, status: true })}
              >
                <Center
                  bg='#white'
                  h={'41px'}
            ...
            <Box>
              ...
              >
                <Center
                  ...
                  borderBottomWidth={'3px'}
                >
                  <Text color={'#6C6C6C'}>Registro</Text>
                </Center>
              </TouchableOpacity>
            </Box>
            <Box>
    ...
    return (
      <View justifyContent={'space-evenly'}>
        <Center>
          <TouchableOpacity
            onPress={() => {
              setisLoad(false), setisLoadV({ vista: 0, status: true });
            }}
          >
            <Center
              bg='#00BAEA'
              ...
              borderRadius={20}
            >
              <Text color={'white'}>Registrate</Text>
            </Center>
          </TouchableOpacity>

          <TouchableOpacity
            onPress={() => {
              setisLoad(false), setisLoadV({ vista: 2, status: true });
            }}
          >
            <Center
              bg='white'
              ...
              borderColor={'#00BAEA'}
            >
              <Text color={'#00BAEA'}>Iniciar sesión</Text>
            </Center>
          </TouchableOpacity>
        </Center>
      </View>
  ...
  function FormularioLogin(){
    ...
    const Validarform = ()  => {
      ...
      if(correo == '' || !correo.match(validRegex)){
        setFormCorreo(true)
        setInputCorreo(true)
        console.log("entro aqui true 1")
      }else{
        setFormCorreo(false)
        setInputCorreo(false)
        console.log("entro aqui false 1")
      }
